...
dfs = {'scaleSrch',1, 'nBit',4, 'simThr',.7, ...
  'dispFlag',1, 'scaleDel',.9, 'rctS',0, 'rctE',-1, 'adaptive', 0 };
...
for frm = 1:nFrame
  ...
  if( scaleSrch )
    ...
    [p,pos,Ic,bestSim]=kernelTracker1(Icur,q,pos1,kernel(2),nBit);
  end

  if prm.adaptive
      [ob,Qc1] = cropWindow( Icur, nBit, pos, wd, ht );
      q = buildHist( Qc1, kernel(scale), nBit );
  end
