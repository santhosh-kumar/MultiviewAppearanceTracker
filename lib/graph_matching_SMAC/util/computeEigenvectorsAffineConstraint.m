...
W:nxn sparse upper triangular matrix.
If you have a symmetric matrix W, type W=sparse(tril(W)); before. It will treat W as if it were symmetric, although more efficiently.

C: kxn constraint matrix
b: kx1 constraint matrix (b cannot be all 0)
nbEigenvectors: # eigenvectors requested
isNcut: 1 or 0 if require normalized cut constrained eigenvectors instead of constrained eigenvectors
...
%     [result,timing] = eigs_optimized(@mex_projection_affine_symmetric,[],nbEigenvectors,options,W,constraint);
else
