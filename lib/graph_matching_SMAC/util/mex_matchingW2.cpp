...
void mexFunction(int nargout, mxArray *out[], int nargin, const mxArray *in[]) {
	...
	graphMatching->deserializeGraphs(mxArray_W1,mxArray_W2,mxArray_I12,mxArray_W12,mxArray_G1,mxArray_G2,mxArray_F12,mxArray_f);
	graphMatching->deserializeOptions(options);
	graphMatching->compute_W();
	graphMatching->normalize_W();

    out[0] = graphMatching->serialize_W();
	if(nargout>=2)
		out[1] = graphMatching->serialize_indi1i2j1j2();
	if(nargout>=3)
		out[2] = graphMatching->serialize_indiW_indjW();
